    ...
    try:
        with urlopen(httprequest) as response:
            ...
            if latest - earliest >= PRUNING:
                ...
                print(
                    f"ERROR: Pruning verification failed: expected to have default pruning of 100 blocks, got: {latest - earliest}, latest {latest}, earliest {earliest}")
    except Exception as ex:
...
def test_eth_chain_id(base_url, test_values):
    ...
    httprequest = Request(url,
                          json.dumps({"jsonrpc": "2.0", "method": "eth_chainId", "params": [], "id": 1}).encode(
                              'utf-8'),
    ...
    try:
        ...
        print(f'ERROR: failed getting eth_chainId from eth json-rpc api: {ex}')
        return
...
def test_eth_pruning(base_url, test_values):
    ...
    try:
        ...
        print(f'ERROR: failed getting eth_blockNumber from eth json-rpc api: {ex}')
        return
    httprequest = Request(url,
    ...
    try:
        ...
        print(f'ERROR: failed getting eth_getBlockByNumber from eth json-rpc api: {ex}')
        return
...
def test_eth_net_api_set(base_url, test_values):
    url = f"{base_url}"
    httprequest = Request(url,
                          json.dumps({"jsonrpc": "2.0", "method": "net_version", "params": [], "id": 1}).encode(
                              'utf-8'),
                          headers=POST_HEADERS,
                          method='POST')
    try:
        ...
        print(f'ERROR: failed getting net_version from eth json-rpc api: {ex}')
        return
...
def test_web3_api_set(base_url, test_values):
    url = f"{base_url}"
    httprequest = Request(url,
                          json.dumps({"jsonrpc": "2.0", "method": "web3_clientVersion", "params": [], "id": 1}).encode(
                              'utf-8'),
                          headers=POST_HEADERS,
                          method='POST')
    try:
        ...
        print(f'ERROR: failed getting web3_clientVersion from eth json-rpc api: {ex}')
        return
